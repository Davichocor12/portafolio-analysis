...
def render_pvt_sector_analysis(df: pd.DataFrame):
    ...
    chart = (
        alt.Chart(summary)
        .mark_bar(color=BRAND_COLORS["soft-secondary"])
        .encode(
            x=alt.X("Exposición:Q", title="Exposición (US$)"),
...
def render_breakdown(df, column, title, label, include_pie=True, show_table=False):
    ...
    if include_pie:
        with col2:
            ...
            palette = [BRAND_PALETTE[i % len(BRAND_PALETTE)] for i in range(len(domain))]
...
def render_heatmap(df):
    ...
    heat = (
        ...
        .encode(
            ...
            color=alt.Color(
                "US $ Equiv:Q",
                scale=alt.Scale(range=BRAND_HEAT_GRADIENT),
            ),
...
def render_orr_heatmap(df):
    ...
    heat = (
        ...
        .encode(
            ...
            color=alt.Color(
                "ORR ponderado:Q",
                scale=alt.Scale(range=BRAND_ACCENT_GRADIENT),
            ),
            tooltip=[
                "Country",
                "Sector",
...
def render_orr_by_dimension(df):
    ...
    chart = (
        alt.Chart(grouped)
        .mark_bar(color=BRAND_COLORS["highlight"])
        .encode(
...
def render_exposure_by_dimension(df):
    ...
    chart = (
        alt.Chart(exposure)
        .mark_bar(color=BRAND_COLORS["soft-primary"])
        .encode(
            x=alt.X("Exposición:Q", title="Exposición (US$)"),
            y=alt.Y(f"{dimension}:N", sort="-x", title=dims[dimension]),
